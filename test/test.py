...
def _assemble_matrices(
    ...
    Returns:
        sps.sparray: Discretization of the face terms as a block matrix. The first block
            row contains the symmetricSecondOrderTensor terms, the second the rotation terms, and the third
            the solid mass 'flux'.
        sps.sparray: Discretization of the boundary conditions, as a map from numerical
            values for the boundary condition to symmetricSecondOrderTensores, rotations and solid mass
...
def _solve(
    ...
    print(face_discretization.shape)
    print(A.shape)
...
if __name__ == "__main__":
    ...
    #bound_symmetricSecondOrderTensor = data[pp.DISCRETIZATION_MATRICES][KEYWORD][
    ...
    num_rot_cell = g.num_cells if g.dim == 2 else 3 * g.num_cells

    # Rotation
    r = x[g.dim * g.num_cells: g.dim * g.num_cells + num_rot_cell]
    print(r.shape)

    # Solid pressure
    p = x[g.dim * g.num_cells + num_rot_cell:]
    print(p.shape)
